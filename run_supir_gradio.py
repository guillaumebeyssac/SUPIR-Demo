    ...
    with gr.Blocks(title="SUPIR Detailer/Enhancer/Upscaler", theme=custom_theme,  
        ...
        with gr.Tabs() as tabs:
            ...
            with gr.TabItem("2. SUPIR", id=1):
                ...
                | `Encoder Tile Size` | Tile size when encoding. <br> TileVAE code has recommended tile sizes based on available VRAM if a CUDA device is available. (but keep in mind you have several models loaded) <br>Encoder: <br>- VRAM > 16GB: 3072<br>- VRAM > 12GB: 2048<br>- VRAM > 8GB: 1536<br>- VRAM <= 8GB: 960<br>- No GPU: 512 |
                | `Decoder Tile Size` | Tile size when encoding. <br> TileVAE code has recommended tile sizes based on available VRAM if a CUDA device is available. (but keep in mind you have several models loaded) <br>Decoder: <br>- VRAM > 30GB: 256<br>- VRAM > 16GB: 192<br>- VRAM > 12GB: 128<br>- VRAM > 8GB: 96<br>- VRAM <= 8GB: 64<br>- No GPU: 64 |
                ...
                | `CFG Sweep Direction` | Defines how the start and end value pairs are varied during a sweep. <br>- Forward: The start value increases while the end value stays fixed. Example: 2/8 → 3/8 → 4/8 → 5/8 ...<br>- Backward: The end value decreases while the start value stays fixed. Example: 2/8 → 2/7 → 2/6 → 2/5 ... |
